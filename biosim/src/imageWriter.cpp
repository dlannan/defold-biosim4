...
static void saveFrameThread(void * _ctx)
{
    ImageWriter * imageWriter = (ImageWriter *)_ctx;
    imageWriter->busy = false; // we're ready for business
    std::cout << "Imagewriter thread started." << std::endl;

    while (true) {
        // wait for job on queue
        std::unique_lock<std::mutex> lck(imageWriter->mutex_);
        imageWriter->condVar.wait(lck, [&]{ return imageWriter->dataReady && imageWriter->busy; });
        // save frame
        imageWriter->dataReady = false;
        imageWriter->busy = false;

        if (imageWriter->abortRequested) {
            break;
        }
